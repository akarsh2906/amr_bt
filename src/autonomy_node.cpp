...
const std::string bt_xml_dir =
    ament_index_cpp::get_package_share_directory("amr_bt") + "/bt_xml";
...
void AutonomyNode::create_behavior_tree()
{
  ...
  RCLCPP_INFO(this->get_logger(), "Registered GoToPose node");


  factory.registerNodeType<PickItem>("PickItem");
//   factory.registerNodeType<AlwaysFailure>("AlwaysFailure");
  factory.registerNodeType<DropItem>("DropItem");
  ...
  // SimulateCharging Node
    factory.registerBuilder<SimulateCharging>("SimulateCharging", 
        [this](const std::string &name, const BT::NodeConfiguration &config) {
            return std::make_unique<SimulateCharging>(name, config, shared_from_this());
    });
